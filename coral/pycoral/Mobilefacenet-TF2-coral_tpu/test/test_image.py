...
img_rahil = preprocess(plt.imread("../image/rahil.jpg"))
img_test = preprocess(plt.imread("../image/lm_2.jpg"))
img_ben = preprocess(plt.imread("../image/ben.jpg"))
img_elizabeth = preprocess(plt.imread("../image/elizabeth.jpg"))
...
if __name__ == '__main__':
    ...
    embedding_rahil = model.predict(img_rahil)
    embedding_ben = model.predict(img_ben)
    embedding_elizabeth = model.predict(img_elizabeth)
   ...
   # embedding_yzy = embedding_yzy / np.expand_dims(np.sqrt(np.sum(np.power(embedding_yzy, 2), 1)), 1)
    ...
    embedding_rahil = embedding_rahil / np.expand_dims(np.sqrt(np.sum(np.power(embedding_rahil, 2), 1)), 1)
    embedding_test = embedding_test / np.expand_dims(np.sqrt(np.sum(np.power(embedding_test, 2), 1)), 1)
    embedding_ben = embedding_ben / np.expand_dims(np.sqrt(np.sum(np.power(embedding_ben, 2), 1)), 1)
    embedding_elizabeth = embedding_elizabeth / np.expand_dims(np.sqrt(np.sum(np.power(embedding_elizabeth, 2), 1)), 1)
    # get result
   # print(np.sum(np.multiply(embedding_yzy, embedding_test), 1))
    print(np.sum(np.multiply(embedding_lm, embedding_test), 1))
    print(np.sum(np.multiply(embedding_rahil, embedding_test), 1))
    print(np.sum(np.multiply(embedding_ben, embedding_test), 1))
    print(np.sum(np.multiply(embedding_elizabeth, embedding_test), 1))
    print(np.sum(np.multiply(embedding_elizabeth, embedding_rahil), 1))
    print(np.sum(np.multiply(embedding_rahil, embedding_ben), 1))
    print(np.sum(np.multiply(embedding_elizabeth, embedding_elizabeth), 1))
    print(embedding_rahil)
    # save database
    db = np.concatenate((embedding_ben, embedding_rahil, embedding_elizabeth), axis=0)
    print(db.shape)

...
def face_recognition(feature, database, threshold=0.5, label_dict=None):
    ...
    feature_norm = feature / np.linalg.norm(feature, 2, -1, keepdims=True)
    
    # Calculate similarities with all database entries
    similarities = np.dot(database, feature_norm.T).squeeze()
    ...
    for idx in top_10_indices:
        if similarities[idx] > threshold:
            # Get person name from label dictionary
            name = label_dict[str(idx)]
            valid_names.append(name)
    
    if not valid_names:
        return -1, 0.0  # No valid matches found
    print("valid names: ", valid_names)
    # Count occurrences of each person's name
    name_counts = Counter(valid_names)
    most_common = name_counts.most_common(1)[0]
    most_common_name = most_common[0]
    confidence = most_common[1] / len(valid_names)
    update_counts(most_common_name)
    # counts.json contains a key for the name, and the value is the count. 

    # if counts.json does not exist, create it
    # if name is in the file already, increment the count
    # if most_common_name is not in the file, add it with count 1
    # Find first index associated with most common name
    for idx in top_10_indices:
        if label_dict[str(idx)] == most_common_name:
            return idx, confidence
            
    return -1, 0.0  # Shouldn't reach here if we found valid matches

def update_counts(name, counts_path="counts.json"):
    """Update face detection counts in JSON file"""
    try:

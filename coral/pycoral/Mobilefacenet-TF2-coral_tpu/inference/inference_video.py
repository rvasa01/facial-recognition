...
if __name__ == '__main__':
    ...
    #if not cap.isOpened():
     #   raise ValueError(f"Failed to open video file: {args.input}")
    # kalman filter
    ...
    while True:
        ...
        if len(pred_prob) > 0:
            ...
            for i in (range(pred_prob.shape[0])):
                ...
                result = face_recognition(feature, rec_db, 0.5, label)
                print(result)
                if args.mask:
        ...
        if loopcount > 0:
            print("percent frames identified: ", identified / loopcount * 100)
